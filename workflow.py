...
def jwt(user: str):
    response = requests.post(
        URL, json={"name": user}, headers={"Content-Type": "application/json"}
    )
    ...
    key = KEY.format(user)
    prv_key = rsa.PrivateKey.load_pkcs1(open(key).read())
    jwt = rsa.decrypt(encrypted_jwt, prv_key)
    return jwt


def vault_token(jwt: str, role: str):
    payload = {"jwt": jwt.decode("utf-8"), "role": role}
    headers = {
...
def infra_username_password(vault_token: str):
    url = f"{VAULT_ADDR}/v1/secret/data/kv/users/alice/mysql_infra_db"
